...
def generate_market_intelligence(session_key: str) -> str:
  ...
  system_instruction = (
      ...
      "Detail the Quad-Ensemble ML architecture: Temporal Fusion Transformer"
      " (TFT) for multi-horizon forecasting, CatBoost for categorical splits,"
      " Random Forest (RF) for node purity and baseline stability, and XGBoost"
      " (XGB) for gradient-boosted loss optimization. Detail BIRCH unsupervised"
      " clustering for microstructure volatility regimes, and VADER for news"
      " sentiment velocity. Discuss regression residuals, R-squared values for"
      " large-cap ETFs (SPY, QQQ), implied 12-month baseline projections (+10%"
      " to +14%), localized NLP sentiment volatility, and selective alpha"
      " opportunities.\n\n"
